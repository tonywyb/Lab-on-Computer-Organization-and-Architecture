...
void renew(Cache& l)
{
	...
	int block_num = cc.block_num;
	uint64_t s = log2(set_num);
	uint64_t b = log2(block_num);
	Storage* lower = l.GetLower();
	Set* set = l.GetSet();
	for(int i=0;i<set_num;++i)
	{
		for(int j =0; j<associativity;++j)
		{
			...
			{
				char *new_content = (char *)set[i].line[j].block;
				uint64_t new_addr = (set[i].line[j].tag << (s + b)) + (i << b);
				lower->HandleRequest(new_addr, block_num, 0, new_content, renew_hit, renew_cycle);
			}
		}
	}
}
...
void init_cache()
{
	...
	s.access_counter = 0;
  	s.miss_num = 0;
 	s.access_cycle = 0; // In nanoseconds
  	s.replace_num = 0; // Evict old lines
  	s.fetch_num = 0; // Fetch lower layer
  	s.prefetch_num; // Prefetch
	mem.SetStats(s);
	l1.SetStats(s);
	l1_inst.SetStats(s);
	l2.SetStats(s);
	...
	CacheConfig stat1(32768, 64, 8, 0, 1);
	CacheConfig stat2(262144, 512, 8, 0, 1);
	CacheConfig stat3(8388608, 16384, 8, 0, 1);
	l1.SetConfig(stat1);
	l1_inst.SetConfig(stat1);
	l2.SetConfig(stat2);
	l3.SetConfig(stat3);
...
void print_res()
{
	for(int i=0; i<6; i++)
	{
		l1.HandleRequest(res_adr + 4 * i, 4, 1, content, renew_hit, renew_cycle);
		res[i] = *((int*)content);
	}
	for(int i=0; i<6; i++)
	{
	...
	printf("\n");
	l1.HandleRequest(a_adr, 4, 1, content, renew_hit, renew_cycle);
	a = *((int*)content);
	l1.HandleRequest(b_adr, 4, 1, content, renew_hit, renew_cycle);
	b = *((int*)content);
	l1.HandleRequest(c_adr, 4, 1, content, renew_hit, renew_cycle);
	c = *((int*)content);
	l1.HandleRequest(sum_adr, 4, 1, content, renew_hit, renew_cycle);
	sum = *((int*)content);
	l1.HandleRequest(temp_adr, 4, 1, content, renew_hit, renew_cycle);
	temp = *((int*)content);
...
void load_memory()
{
	...
	unsigned int st = madr;
	memcpy(mem.GetMemAddr(), memory, 400000000);
/*
	for(int i=st;i<st+400;i+=4)
	{
		cout << "inst" << i/4 <<endl;
		char* dest = mem.GetMemAddr();
		int tmp=*((int*)(dest+i));
		cout << "0x"<< tmp <<endl;
	}
*/
...
int main(int argc, char **argv)
{
	...
	simulate();
	cache2mem();
	cout << "after push back to memory, the results are:"<<endl;
	print_res();
	printf("===============================\n");
	inst_num += 2;
	cout <<"simulate over!"<<endl;
    cout << "cycle number is:" << new_cycle << endl;
    cout << "inst number is:" << inst_num << endl;
    cout << "CPI is:"<< (double)new_cycle/inst_num << endl;
	cout << "hit time is:" << hit << endl;
	cout << "miss time is:" << readcount + writecount - hit <<endl;
	cout << "read time is:" << readcount <<endl;
	cout << "write time is:" << writecount <<endl;
	cout << "read/write time is:" << readcount + writecount <<endl;
	int all_count = readcount + writecount;
	//double miss_count = (double)miss/all_count;
	//printf("miss rate is:%lf\n", miss_count);
	cout << "miss rate is:" << 1 - (double)hit/all_count <<endl;
...
{
	readcount ++;
	printf("inst_num %d & cycle %d fetch, PC:0x%x\n", inst_num, cycle, PC*4);
	addr = (uint64_t)(PC * 4);
	l1_inst.HandleRequest(addr, 4, 1, content, hit, new_cycle);	//read
	IF_ID_old.inst = *((unsigned int*)content);
...
void ID()
{
	printf("inst_num %d & cycle %d decode, PC:0x%x\n",inst_num, cycle, IF_ID.PC*4);
...
void EX()
{
    ...
    if (ID_EX.PC != -1)
    {
		...
		printf("inst_num is :%d\n", inst_num);
        printf("its PC is:", ID_EX_old.PC, '\n');
    }

	printf("inst_num %d & cycle %d execute, PC:0x%x\n", inst_num, cycle, ID_EX.PC*4);
	/*if (ID_EX.PC == endPC)
        exit_flag = 1;*/
	//read ID_EX
	...
	switch(ALUOp)
	{
		...
		case ALU_ADD:
			...
			break;
		case ALU_MUL:
			ALUout = input1 * input2;cycle++;new_cycle++;break;
		case ALU_SUB:
			ALUout = input1 - input2;
			if(Branch == BEQ)
			{
		...
		case ALU_XOR:
			ALUout = input1 ^ input2;break;
		case ALU_DIV:
			ALUout = input1 / input2;cycle+=39;new_cycle+=39;break;
		case ALU_SRL:
...
void MEM()
{
	printf("inst_num %d & cycle %d MEM, PC:0x%x\n",inst_num, cycle, EX_MEM.PC*4);
	//read EX_MEM
	...
	switch(MemRead)
	{
		case 0:
			break;
		case 1://byte
			l1.HandleRequest(ALUout, 1, 1, content, hit, new_cycle);	//read
			//temp = *((char*)((unsigned char*)memory + ALUout));
			temp = *((char*)content);
			Mem_read = (long long)temp;
			break;
		case 2://half
			l1.HandleRequest(ALUout, 2, 1, content, hit, new_cycle);	//read
			//temp1 = *((short*)((unsigned char*)memory + ALUout));
			temp1 = *((short*)content);
			Mem_read = (long long)temp1;
			break;
		case 3://word
			l1.HandleRequest(ALUout, 4, 1, content, hit, new_cycle);	//read
			//temp2 = *((int*)((unsigned char*)memory + ALUout));
			temp2 = *((int*)content);
			Mem_read = (long long)temp2;
			break;
		case 4://double word
			l1.HandleRequest(ALUout, 8, 1, content, hit, new_cycle);	//read
			//temp3 = *((long long*)((unsigned char*)memory + ALUout));
			temp3 = *((long long*)content);
	...
	switch(MemWrite)
	{
		case 0:
			break;
		case 1://byte
			*((char*)content) = (char)Rs2;
			l1.HandleRequest(ALUout, 1, 0, content, hit, new_cycle);	//write
			//Rs2 = (char)Rs2;
			//*((char*)((unsigned char*)memory + ALUout)) = Rs2;
			
			break;
		case 2://half
			*((short*)content) = (short)Rs2;
			l1.HandleRequest(ALUout, 2, 0, content, hit, new_cycle);	//write
			//Rs2 = (char)Rs2;
			//*((char*)((unsigned char*)memory + ALUout)) = Rs2;
			break;
		case 3://word
			*((int*)content) = (int)Rs2;
			l1.HandleRequest(ALUout, 4, 0, content, hit, new_cycle);	//write
			//Rs2 = (char)Rs2;
			//*((char*)((unsigned char*)memory + ALUout)) = Rs2;
			break;
		case 4://double word
			*((long long*)content) = (long long)Rs2;
			l1.HandleRequest(ALUout, 8, 0, content, hit, new_cycle);	//write
			//Rs2 = (char)Rs2;
			//*((char*)((unsigned char*)memory + ALUout)) = Rs2;
			break;
	}
...
void WB()
{
	printf("inst_num %d & cycle %d WriteBack, PC:0x%x\n",inst_num, cycle, MEM_WB.PC*4);
