...
void Cache::HandleRequest(uint64_t addr, int bytes, int read,
	...
	if (!BypassDecision()) {
		...
		if (PrefetchDecision(addr, replace_decision)) {
...
void Cache::ReplaceAlgorithm(uint64_t addr, int bytes, int read,
	...
	if ((read == 0 || read == 2) && config_.write_allocate == 0)
	{
...
int Cache::PrefetchDecision(uint64_t addr,int replace_decision) {
  return FALSE;
}

void Cache::PrefetchAlgorithm(uint64_t addr,int replace_decision) {
}
